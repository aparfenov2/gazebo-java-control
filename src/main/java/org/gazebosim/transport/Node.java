...
public class Node implements Runnable, ServerCallback {
	...
	public final Connection master = new Connection();
...
//	}
	...
	public void unadvertize(String topic) {
		topic = fixTopic(topic);
		LOG.info("UNADV "+topic);
		Publish req = Publish.newBuilder().setTopic(topic).setMsgType("")
		  	.setHost(server.host).setPort(server.port).build();
		try {
			master.writePacket("unadvertise", req);
		} catch (IOException e) {
			e.printStackTrace(); // FIXME: Shouldn't happen, should probably complain louder
		}
	...
	public synchronized void processPacket(Packet packet) throws InvalidProtocolBufferException {
		...
		} else if (packet.getType().equals("publisher_del")) {
			Publish pubr = Publish.parseFrom(packet.getSerializedData());
			PublisherRecord pub1 = publishers.get(pubr.getTopic());
			if (pub1 != null) {
				LOG.info("close publisher "+pub1);
				pub1.close();
				publishers.remove(pubr.getTopic());
			}
